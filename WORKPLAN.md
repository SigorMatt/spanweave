...
- 2026-09-19: S10 widened past its row, deliberately: absent/`null`/non-string
  link targets, a non-object link entry and a non-list `links` now draw
  `unmapped_attributes` too (all were silently dropped; none in the corpus),
  since SPEC treats absent, `null` and `""` as one statement. New scenario
  `empty_link_target` (FIXTURES §8 froze `empty_ids`); census 56/159. For the
  cold review: accept the widening or narrow it.
- 2026-09-19: S11 re-measured the ceilings (3.11.15 994/994, 3.12.3
  9997/9997, 3.13.14 9998/9998; 3.11 varies with caller depth, so the table's
  dated 992 stands). Left for S12: TASKS.md run-4 map row 13 still carries
  the "±2" dismissal -- point it at thread 10.
